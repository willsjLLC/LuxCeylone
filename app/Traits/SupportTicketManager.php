...
trait SupportTicketManager
{
    ...
    public function ticketDownload($attachmentId)
    {
        try {
            ...
            // Better mime type detection with fallback
            try {
                $mimetype = mime_content_type($fullPath);

                // Fallback for common file types if mime_content_type returns false or application/octet-stream
                ...
                // If mime detection fails completely, use a generic binary type
            ...
            if ($this->apiRequest) {
                return response()->json([
                    'remark' => 'download_error',
                    'status' => 'error',
                    'message' => ['error' => $e->getMessage()],
                ]);
            }

            $notify[] = ['error', 'An error occurred while downloading: ' . $e->getMessage()];
            return back()->withNotify($notify);
        }
    }
}
