...
class NotificationController extends Controller
{
    public function globalEmail()
    {
        ...
        if (!$admin || !$admin->can('settings.notification_settings')) {
    ...
    public function emailTest(Request $request)
    {
        ...
        try {
            Notification::route('mail', $request->email)
                ->notify(new TestMailNotification($subject, $message, $receiverName));

            $notify[] = ['success', 'Email sent to ' . $request->email . ' successfully'];
        } catch (\Exception $e) {
            Log::error('Email sending failed: ' . $e->getMessage());
            $notify[] = ['error', 'Email sending failed: ' . $e->getMessage()];
        }
