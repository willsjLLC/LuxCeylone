...
@section('panel')
    <div class="col-12">
        <div class="row">
            <h3 class="mb-4 fw-bold text-dark mt-10">Job History</h3>
            <!-- Sidebar (Left) -->
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @if (strlen($job->title) > 20)
                                                <br>
                                                <small class="jobTitle text--base "
                                                    data-title_details="{{ __($job->title) }}">@lang('Read More')</small>
                                            @endif
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <div class="text--end">
                                                <span
                                                    class="time">{{ showDateTime($job->created_at, 'M d, Y h:i:s a') }}<br>{{ diffForHumans($job->created_at) }}</span>
                                            </div>
                                        </td>
                                        <td>
                                            @php
                                            ...
                                            <div class="d-flex flex-wrap table-icon">
                                                <a href="{{ !$isEnable ? route('user.job.edit', $job->id) : 'javascript:void(0)' }}"
                                                    class="text--success me-2 {{ $isEnable ? 'disabled' : '' }} "
                                                    @if (!$isEnable) data-bs-toggle="tooltip" title="Edit" @endif>
                                                    <i class="lar la-edit"></i>
                                                </a>
                                                @if ($job->status == Status::JOB_PAUSE)
                                                    <a href="JavaScript:void(0)"
                                                        class="text--base statusBtn me-2 confirmationBtn"
                                                        data-bs-toggle="tooltip"
                                                        data-action="{{ route('user.job.status', $job->id) }}"
                                                        data-question="@lang('Are you sure to approve this job ')" title="@lang('Approve')">
                                                        <i class="las la-check-circle"></i>
                                                    </a>
                                                @elseif($job->status == Status::JOB_APPROVED)
                                                    <a href="JavaScript:void(0)"
                                                        class="text--warning statusBtn me-2 confirmationBtn"
                                                        data-bs-toggle="tooltip"
                                                        data-action="{{ route('user.job.status', $job->id) }}"
                                                        data-question="@lang('Are you sure to paused this job ')" title="@lang('Paused')">
                                                        <i class="las la-pause-circle"></i>
                                                    </a>
                                                @endif

                                                <a href="{{ route('user.job.details', $job->id) }}"
                                                    class="text--info notification-holder" data-bs-toggle="tooltip"
                                                    title="@lang('Job prove')">
