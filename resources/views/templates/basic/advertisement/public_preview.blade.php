        ...
        <div class="row">
                    ...
                    <div class="mb-3">
                        ...
                        @auth
                            @if(auth()->user()->id == $advertisement->user_id)
                                ...
                                @else
                                    <a href="{{ route('user.login', $advertisement->id) }}" class="boost-ad-btn">
            ...
            <div class="col-md-4">
                ...
                <div class="mb-4 seller-contact">
                    ...
                    <div class="seller-info">
                                ...
                                <div class="d-flex flex-column">
                                    ...
                                    @php
                                        ...
                                        if (strlen($primaryPhone) == 10) {
                                            $formattedPrimaryPhone = substr($primaryPhone, 0, 3) . ' - ' . substr($primaryPhone, 3);
                                        } else {
                                            $formattedPrimaryPhone = $primaryPhone;
                                        }

                                        if (strlen($optionalPhone) == 10) {
                                            $formattedOptionalPhone = substr($optionalPhone, 0, 3) . ' - ' . substr($optionalPhone, 3);
                                        } else {
                        ...
                        <div class="mb-3 contact-details">
                            ...
                            <div class="contact-value1">{{ $advertisement->contact_email }}</div>
                        </div>
                ...
                <div class="mb-4 ad-status-actions">
                    ...
                    @if($advertisement->status == \App\Constants\Status::AD_APPROVED || $advertisement->status == \App\Constants\Status::AD_PENDING)
                        <div class="action-buttons">
                            @if($advertisement->status == \App\Constants\Status::AD_APPROVED)
                                <button type="button" class="btn action-btn complete-btn" data-bs-toggle="modal" data-bs-target="#completeConfirmationModal">
                                    <i class="fa fa-check-circle me-2"></i> Mark as Completed
                                </button>
                            @endif
                            <button type="button" class="btn action-btn cancel-btn" data-bs-toggle="modal" data-bs-target="#cancelConfirmationModal">
                ...
                <div class="mb-4 share-options">
                    ...
                    <div class="share-buttons">
                        <a href="{{ route('user.login') }}" target="_blank" class="share-btn facebook-btn">
                            <i class="fab fa-facebook-f"></i>
                        </a>

                        <a href="{{ route('user.login') }}" target="_blank" class="share-btn twitter-btn">
                            <i class="fab fa-twitter"></i>
                        </a>

                        <a href="{{ route('user.login') }}" target="_blank" class="share-btn whatsapp-btn">
                            <i class="fab fa-whatsapp"></i>
                        </a>

                        <a href="{{ route('user.login') }}" target="_blank" class="share-btn instagram-btn">
                            <i class="fab fa-instagram"></i>
                        </a>

                        <a href="{{ route('user.login') }}" target="_blank" class="share-btn tiktok-btn">
                            <i class="fab fa-tiktok"></i>
                        </a>

                        <a href="{{ route('user.login') }}" target="_blank" class="share-btn youtube-btn">
                            <i class="fab fa-youtube"></i>
                        </a>

                        <a href="{{ route('user.login') }}" class="share-btn email-share-btn">
            ...
            <div class="mb-4 related-ads ">
                ...
                <div class="row ">
                    @foreach ($relatedAds as $relatedAd)
                        @php
                            $isBoosted = isset($relatedAd->latestBoostHistory) && $relatedAd->latestBoostHistory && $relatedAd->latestBoostHistory->boostPackage;
                            $boostType = $isBoosted ? $relatedAd->latestBoostHistory->boostPackage->type : null;
                            $highlightColor = $isBoosted ? $relatedAd->latestBoostHistory->boostPackage->highlighted_color : null;
                            $cardClass = $isBoosted ? 'ad-card related-ad-card boosted-card' : 'ad-card related-ad-card';
                            $styleAttr = $highlightColor ? "border: 2px solid {$highlightColor};" : '';
                        @endphp
                        <div class="mb-3 col-12 col-md-6 col-lg-3">
                            <a href="{{ route('user.login') }}" class="text-decoration-none text-dark">
                                <div class="{{ $cardClass }}" data-ad-id="{{ $relatedAd->id }}" style="{{ $styleAttr }}">
                                    ...
                                    <div class="card-container">
                                        <div class="card-details">
                                            <p class="card-title">{{ $relatedAd->title }}</p>
                ...
                <div class="modal-body">
                    ...
                    @if($advertisement->status == \App\Constants\Status::AD_COMPLETED)
                        <div class="sold-tag-overlay-modal">
                            <img src="{{ asset('assets/images/sold-tag.png') }}" alt="SOLD" class="sold-tag-image-modal">
                        </div>
                    @endif
                </div>
...
@push('script')
    <style>
        ...
        .ad-preview-image {
            ...
            display: none;
            cursor: pointer;
            position: absolute; /* Added to ensure images fully cover */
            top: 0;
            left: 0;
        }
        ...
        .thumbnails {
            ...
            gap: 15px; /* Increased gap between thumbnails */
            padding: 8px 0;
        ...
        .thumbnail {
            flex: 0 0 120px; /* Increased size from 80px */
            height: 90px; /* Increased height from 60px */
            border-radius: 6px;
            overflow: hidden;
            cursor: pointer;
    ...
    <script>
        $(document).ready(function() {
            ...
            let currentImage = 0;
            const totalImages = $('.thumbnail').length;
            const imageSources = [];
            const imageAlts = [];
            ...
            // Handle main image click - show modal
            $('.ad-preview-image').on('click', function() {
                const index = $(this).data('index');
            ...
            function openImageModal(index) {
                $('#modalImage').attr('src', imageSources[index]);
                $('#modalImage').attr('alt', imageAlts[index]);
            ...
            function navigateModalImages(direction) {
                ...
                $('#modalImage').attr('src', imageSources[currentImage]);
                $('#modalImage').attr('alt', imageAlts[currentImage]);
                updateModalCounter(currentImage);
