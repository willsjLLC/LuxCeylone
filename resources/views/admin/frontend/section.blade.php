                                ...
                                @foreach ($section->content as $k => $item)
                                    @if ($k == 'images')
                                        ...
                                        @foreach ($item as $imgKey => $image)
                                            <div class="col-md-4">
                                                @if (__(keyToTitle(@$imgKey)) == 'Banner image')                                                    
                                                <input type="hidden" name="has_image" value="1">
                                                <div class="form-group">
                                                    <label>{{ __(keyToTitle(@$imgKey)) }}</label>
                                                    <x-image-uploader class="w-100" name="image_input[{{ @$imgKey }}]" :imagePath="frontendImage($key,@$content->data_values->$imgKey,@$section->content->images->$imgKey->size)" id="image-upload-input{{ $loop->index }}" :size="'940x788'" :required="false" />
                                                </div>
                                                @else
                                                <input type="hidden" name="has_image" value="1">
                                                <div class="form-group">
